...
| 標記（第一行，逐字） | 何時貼 | 誰讀它 |
| --- | --- | --- |
| `<!-- pr-review-response:start -->` | 步驟 2，接手的第一個動作 | `pr-review-poller` 算 `T_mark`；步驟 1 的並行防護 |
| `<!-- pr-review-response:done -->` | 步驟 7，處理完 | 步驟 1 算「還沒回應的 review」 |
| `<!-- pr-review-response:failed -->` | 任何步驟失敗時（見「失敗處理」） | 人類；步驟 1 的並行防護 |
...
3. **讀 PR 與權限**：
   ...
   - `isCrossRepository` 是 `true`，或 `.permissions.push` 不是 `true` → 推不回原
     分支，Discord 回覆缺什麼權限並結束。
4. **並行防護**：在 issue comments 裡找第一行是 `start` 標記、簽名是自己的留言，
   取最新一則。它在 2 小時內（與 `pr-review-poller` 的 `PR_REVIEW_TTL_HOURS`
   預設值一致）、且之後沒有自己的 `done`／`failed` 留言 → 另一輪還在跑，
   Discord 回覆「另一輪處理中（開始於 <時間>），2 小時後仍無結果可再觸發」並結束。
5. **算出這一輪要處理什麼**，兩種來源：
...
以上任一種「結束」都還沒貼 `start` 標記，所以不需要補 `done`／`failed` 留言。
...
- 每則待處理 review 的 line comments，一則一個項目：
...
- **預設是改**。「問」只留給真的有歧義的意見——每條都先問，人類就得再講一次話，
  這支 skill 就白做了。
- **反駁一定要留回覆**。不認同就寫出理由，讓人類決定；照做一個自己知道有問題的
  改法、或不吭聲地跳過，兩者都不是可接受的結果。
...
3. `git push origin HEAD:<headRefName>`。被拒（遠端有新 commit）→
   `git pull --rebase origin <headRefName>`、重跑測試、再 push 一次；還是失敗或
   rebase 有衝突 → 失敗處理。
...
失敗留言**不列「已處理 review」**，下一輪會重新處理同一批 review；已經 push 的
修改在下一輪讀 diff 時會看到，判斷為已完成即可。
