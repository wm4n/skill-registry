...
`gh pr create` 開 PR。PR body 的署名依 persona 的署名形式決定：

- **persona 署名是 `— By <Bot>`**（Rick、Morty）→ PR body 內容結尾獨立一行寫 PR 簽名，`<Bot>` 代入 persona 署名裡的名字：
...
- **persona 另有署名規定**（Genie 的 `— Instructed by …`）→ 照 persona 署名，不寫 PR 簽名。Genie 的帳號底下沒有別的 dev bot，author 就足以辨識。
