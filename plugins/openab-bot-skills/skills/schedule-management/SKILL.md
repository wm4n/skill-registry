...
- **channel ID**：cron 需要數字 snowflake，你在容器內沒有可靠管道把「當前頻道」解析成 ID。請對方在該頻道右鍵→「複製頻道 ID」（需開 Discord 開發者模式）貼給你；或給你一個 thread ID。
...
- **有冒出來** → 整條鏈通了（usercron 有開、路徑對、channel 對、bot 在該頻道）。把 `verify-ping` 這段刪掉、只留正式 job，再存一次。
