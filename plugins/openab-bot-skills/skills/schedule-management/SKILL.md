...
> **硬性門檻**：`channel`、`timezone`、`message` 這三個欄位，沒有跟人類確認過、或明確採用預設值（僅 `timezone` 可用 `Asia/Taipei` 當預設，`channel`／`message` 不可自己代入猜測），**不准動筆寫 `cronjob.toml`**。寧可多問一次，不要自己腦補。
...
- ⚠️ **只跑一次的測試/一次性任務**（人類說「測一次就好」「5 分鐘後跑一次」這種）：cron 運算式**沒有「年」欄位**，就算用 `date` 算出精確的分/時/日/月填進 `schedule`，到了明年同一天同一分還是會再觸發一次——不是真的只跑一次。**不要**用 `disable_on_success = true` 這種寫法去解決「只跑一次」——那不是合法用法（見下方進階說明），也不會讓它變成一次性。正確做法：跟 §4 的 `verify-ping` 一樣，先建立、等它真的觸發過一次、確認結果 OK 後，**手動把這個 job 刪掉**；不要指望它自己停。
...
- 進階：`disable_on_success` 是給「目標達成自停用」用的（例如「持續修到測試過為止」），**值必須是一段 shell 指令字串**（如 `"npm test && echo DONE"`），且要搭配 `disable_on_success_match` 一起設（指令 exit 0 **且**輸出包含這個字串，才會自動 `enabled = false`）；需要 `id`。**不是**布林值、**不能**單獨設 `= true`，也不適用於單純「跑一次看結果」的測試 job（那種用完手動刪，見 §3）。完整欄位／限制見 repo 的 `docs/cronjob.md` 或 `deployment-guides/CRONJOB.md`。
...
- `channel`／`timezone`／`message` 沒確認過或沒有合理預設前，不准寫檔。
- 一次性測試/任務跑完後**手動刪 job**，別靠 `disable_on_success = true` 這種無效寫法指望它自己停。
